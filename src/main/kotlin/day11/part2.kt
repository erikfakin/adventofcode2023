...
fun main() {
    ...
    for (i in 0..<stars.size) {
        for (j in i + 1..<stars.size) {
            ...
            val xExpansion = ((expansionFactor - 1L) * emptyColsInBetween)
            val yExpansion = ((expansionFactor - 1L) * emptyRowsInBetween)
            val dx = (star2[0] - star1[0]).absoluteValue + xExpansion
            val dy = (star2[1] - star1[1]).absoluteValue + yExpansion
